 ...
 */
package com.apps.adrcotfas.mytime.data.local.migrations
        ...
        override fun migrate(connection: SQLiteConnection) {
            connection.execSQL(
                ...
                CREATE TABLE IF NOT EXISTS plannedTask (
                    id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                    title TEXT NOT NULL,
                    labelName TEXT NOT NULL DEFAULT 'unlabeled',
                    isArchived INTEGER NOT NULL DEFAULT 0,
                    ...
                    notes TEXT NOT NULL DEFAULT '',
                    FOREIGN KEY(labelName, isArchived) REFERENCES localLabel(name, isArchived) ON UPDATE CASCADE ON DELETE SET DEFAULT,
                    FOREIGN KEY(linkedSessionId) REFERENCES localSession(id) ON UPDATE CASCADE ON DELETE SET NULL
                );
                """.trimIndent(),
            )
            connection.execSQL("CREATE INDEX IF NOT EXISTS index_plannedTask_startTime_endTime ON plannedTask(startTime, endTime);")
            connection.execSQL("CREATE INDEX IF NOT EXISTS index_plannedTask_labelName_isArchived ON plannedTask(labelName, isArchived);")
            connection.execSQL("CREATE INDEX IF NOT EXISTS index_plannedTask_linkedSessionId ON plannedTask(linkedSessionId);")
            connection.execSQL("CREATE INDEX IF NOT EXISTS index_plannedTask_isCompleted ON plannedTask(isCompleted);")
            connection.execSQL("CREATE INDEX IF NOT EXISTS index_plannedTask_dayEpoch ON plannedTask(dayEpoch);")
        }
    }
val MIGRATIONS =
    arrayOf(
