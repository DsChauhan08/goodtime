...
class MyTimeLauncherAlias

/**
 * Older versions let users rename the launcher to "Productivity", which disabled
 * [MyTimeLauncherAlias] at runtime. That option (and its alias) is gone now, so re-enable the
 * MyTime launcher for anyone still on the old override — otherwise they'd have no launcher icon.
 */
// runs on every boot/update forever; delete this whole path once "Productivity"
// installs have aged out (safe from ~mid-2027).
fun restoreMyTimeLauncherIfNeeded(context: Context) {
    val pm = context.packageManager
    val alias = ComponentName(context, MyTimeLauncherAlias::class.java)
