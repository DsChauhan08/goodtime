...
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │  mytime_timer_state (DomainTimerData equivalent)                    │   │
│  │  mytime_completed_sessions (array of finished sessions)             │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
                              ▲            ▲
                              │            │
...
struct UserDefaultsKeys {
    static let timerState = "mytime_timer_state"
    static let completedSessions = "mytime_completed_sessions"
}
...
class IosUserDefaultsTimerStorage(
    ...
    private val userDefaults = NSUserDefaults(suiteName = "group.com.apps.adrcotfas.mytime")
    ...
    fun writePendingAction(action: String) {
        userDefaults?.setObject(action, forKey = "mytime_pending_action")
        userDefaults?.synchronize()
    }

    fun readPendingAction(): String? {
        return userDefaults?.stringForKey("mytime_pending_action")
    }

    fun clearPendingAction() {
        userDefaults?.removeObjectForKey("mytime_pending_action")
    }
}

// UserDefaults keys
object UserDefaultsKeys {
    const val timerState = "mytime_timer_state"
    const val completedSessions = "mytime_completed_sessions"
}
```
...
@objc public class TimerStateManager: NSObject {
    ...
    private let userDefaults = UserDefaults(suiteName: "group.com.apps.adrcotfas.mytime")
...
**Modified: `MyTimeAppIntents.swift`**

```swift
struct MyTimeTogglePauseIntent: LiveActivityIntent {
    ...
    private func updateLiveActivityUI(from state: TimerState) async {
        for activity in Activity<MyTimeActivityAttributes>.activities {
        ...
        if state.state == "paused" {
            ...
            if remaining > 0 {
                ...
                let request = UNNotificationRequest(
                    identifier: "mytime_timer_finished",
                    content: content,
                    trigger: trigger
                )
    ...
    private func mapToContentState(_ state: TimerState) -> MyTimeActivityAttributes.ContentState {
        ...
        return MyTimeActivityAttributes.ContentState(
...
// Similar changes for:
// - MyTimeStopIntent
// - MyTimeAddMinuteIntent
// - MyTimeStartBreakIntent (uses .skip action)
// - MyTimeStartFocusIntent (uses .skip action)
```
...
2. Modify `MyTimeAppIntents.swift` to use `TimerStateManager`
...
### From MyTimeLiveActivityManager (iOS)
...
| `MyTimeAppIntents.swift` | Use `TimerStateManager` instead of NotificationCenter; set label when Live Activity starts |
...
| `MyTimeInProgressLiveActivity.swift` | Update UI after state change from AppIntents |
| `MyTimeLiveActivityManager.kt` (or bridge) | Call `TimerManager.shared.setLabel()` when starting Live Activity |
