...
- `mytime-db` — Room DB with the default label's `workDuration=1`.
...
```sh
P=com.apps.adrcotfas.mytime
adb shell pm clear $P
adb shell am start -n $P/.settings.MyTimeLauncherAlias
...
adb exec-out run-as $P cat databases/mytime-db > mytime-db
sqlite3 mytime-db "UPDATE localLabel SET workDuration=1 WHERE name='PRODUCTIVITY_DEFAULT_LABEL'; VACUUM;"
